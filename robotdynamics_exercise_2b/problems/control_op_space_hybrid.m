...
% mu = ... ;
% p =  ... ;
lambda=pseudoInverseMat_solution(I_Je*pseudoInverseMat_solution(M_fun_solution(q),0.01)*I_Je',0.01);
mu=lambda*I_Je*pseudoInverseMat_solution(M_fun_solution(q),0.01)*b_fun_solution(q,q_dot)-lambda*I_dJ_e*q_dot;
p=lambda*I_Je*pseudoInverseMat_solution(M_fun_solution(q),0.01)*g_fun_solution(q);
...
tau = zeros(6,1); % TODO
w_dot=kpMat*chi_err-kdMat*I_dJ_e*dq;
tau=I_Je'*(lambda*Sm*w_dot+Sf*I_F_E+mu+p);
end
