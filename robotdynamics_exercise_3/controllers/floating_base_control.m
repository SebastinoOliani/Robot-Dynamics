...
%% Optimization Tasks
% x_opt = [ddq', f_c_xz', tau'] % dim = 7+4+7 = 18

% Equations of motions
A_eom = [M, -J_c', -S']; % 10x10 4x10 7x10
b_eom = -b-g;
...
% ddq=du
% kp*(q-q_des) = kp*(q-J_B*p_star)
% kd*(dq-dq_des) = kd*(dq-J_B*w_star)
% a_B = dJ_B*dq + J_B * ddq
kp_b = 10; 
kd_b = 2*sqrt(kp_b);
A_b = [J_B, zeros(3,4), zeros(3,7)];
b_b = -dJ_B*dq + kp_b*(p_star_B - p_B) + kd_b*(w_star_B - w_B);

% No end-effector motion
% HINT: kp_ee = 5; kd_ee = 2*sqrt(kp_b);
% kp * (q-q_des) = kp * (q - J_EE*p_star_EE)
% kd * (dq-dq_des) = kd * (dq - J_EE*w_star)
% a_EE = dJ_EE * dq + J_EE * ddq
kp_ee = 5; 
kd_ee = 2*sqrt(kp_ee);
A_ee = [J_EE, zeros(size(J_EE,1),4), zeros(size(J_EE,1),7)];
b_ee = -dJ_EE*dq + kp_ee*(p_star_EE - p_EE) + kd_ee*(w_star_EE - w_EE);
